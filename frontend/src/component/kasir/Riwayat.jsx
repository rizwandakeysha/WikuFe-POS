...
export default function Riwayat() {
  ...
  const handleBayar = async (id_transaksi) => {
    const selectedTransaksi = transaksi.find(
      (select) => select.id_transaksi === id_transaksi
    );
    const selectedMeja = meja.find(
      (select) => select.id_meja === selectedTransaksi.meja.id_meja
    );
    ...
    try {
      await axios.put(
        "http://localhost:8080/transaksi/" + id_transaksi,
        updatedStatusTransaksi,
        { headers }
      );
      await axios.put(
        "http://localhost:8080/meja/" + selectedMeja.id_meja,
        updatedStatusMeja,
        { headers }
      );
  ...
  return (
    <div><br /><br /><br />
        <div class="mb-15  items-center justify-center">
          <h1
                ...
                <td className="px-6 py-4">
                  {transaksi.status === "belum_bayar" ? (
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleBayar(transaksi.id_transaksi)}
                ...
                {transaksi.status === "belum_bayar" ? (
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-4">
                      <button
                        onClick={() => handleRemove(transaksi.id_transaksi)}
                      >
                        <svg
